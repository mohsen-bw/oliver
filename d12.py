...
def myhelp():
    myHelp = """*ทุกคำสั่งมี / นำหน้านะครับ
...
•ตั้งติ๊กแทค [ส่งสติ๊กเกอร์] = ตั้งส่งสติ๊กเกอร์ตอนคนแทค
•ตั้งติ๊กเข้า [ส่งสติ๊กเกอร์] = ตั้งส่งสติ๊กเกอร์ตอนคนเข้า
•ตั้งติ๊กออก [ส่งสติ๊กเกอร์] = ตั้งส่งสติ๊กเกอร์ตอนคนออก
•ตั้งติ๊กเตะ [ส่งสติ๊กเกอร์] = ตั้งส่งสติ๊กเกอร์ตอนคนเตะกัน
•ตั้งติ๊กแอบ [ส่งสติ๊กเกอร์] = ตั้งส่งสติ๊กเกอร์คนแอบอ่าน
•ติ๊กแทค = ดูสติ๊กเกอร์ที่ตั้งไว้ตอนคนแทค
•ติ๊กเข้า = ดูสติ๊กเกอร์ที่ตั้งไว้ตอนคนเข้า
•ติ๊กออก = ดูสติ๊กเกอร์ที่ตั้งไว้ตอนคนออก
•ติ๊กเตะ = ดูสติ๊กเกอร์ที่ตั้งไว้ตอนคนเตะ
•ติ๊กแอบ = ดูสติ๊กเกอร์ที่ตั้งไว้ตอนคนแอบอ่าน
•แทค = แทคคนทั้งห้อง
...
#==============================================================================#
                    ...
                    link = "http://line.me/ti/p/~botline2034"
                    link = "http://line.me/ti/p/~l581-1"
                    line.sendFooter(to, "『◉⊰ᎢᎬᎪᎷ Ᏼ⦿Ꭲ🔰ᎠᎬᏞᎬᎢ⊱◉』",link ,icon,name)
                    line.sendFooter(to, "กำลังโหลด:▒...0%",link ,icon,name)
                    line.sendFooter(to, "█▒... 10.0%" ,link ,icon,name)
                    line.sendFooter(to, "██▒... 20.0%" ,link ,icon,name)
                    line.sendFooter(to, "███▒... 30.0%" ,link ,icon,name)
                    line.sendFooter(to, "████▒... 40.0%" ,link ,icon,name)
                    line.sendFooter(to, "█████▒... 50.0%" ,link ,icon,name)
                    line.sendFooter(to, "██████▒... 60.0%" ,link ,icon,name)
                    line.sendFooter(to, "██████▒... 70.0%" ,link ,icon,name)
                    line.sendFooter(to, "████████▒... 80.0%" ,link ,icon,name)
                    line.sendFooter(to, "█████████▒... 90.0%" ,link ,icon,name)
                    line.sendFooter(to, "███████████..100.0%" ,link ,icon,name)
                    line.sendFooter(to, " https://youtu.be/LMG_-P-MNg8 " ,link ,icon,name)
                elif text.lower() == '/แจก':
...
#==============================================================================#               
                   ...
                   line.sendFooter(to, "[ %s Seconds ] [ " % (elapsed_time) + str(int(round((time.time() - start) * 1000)))+" ms ]",link ,icon,name2)
                    ...
                    line.sendContact(to, "u7ec885ad82b3ff28a1c2ac445c6bd5f2")                
...
#==============================================================================#
                    ...
                    text = msg.text.lower().replace("/แทคแชท2: ","")
...
#==============FINNISHING PROTECT========================#
                elif msg.text.lower() == '/เปิดรับเข้า':
                        if settings["Wc"] == True:
                            if settings["lang"] == "JP":
                            ...
                            if settings["lang"] == "JP":
                                line.sendMessage(to,"เปิดข้อความต้อนรับเมื่อมีสมาชิกเข้ากลุ่ม(○ﾟεﾟ○)")
                elif msg.text.lower() == '/ปิดรับเข้า':
                ...
                elif msg.text.lower() == '/เปิดส่งออก':
                        if settings["Lv"] == True:
                            if settings["lang"] == "JP":
