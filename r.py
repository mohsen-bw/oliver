...
#ki = LINE("ExJv4eANpdJRnh4pQvFf.s0F19RdwonJdLC5Lh1l7BW.MhXZdM6NUUn7WjmfAaPoDr2ldtrggK/rdsLtA/1gJwE=")
#kk = LINE("ExAot7f4dytp50889WMd.CQ21dgoQa8cJvr/gfRG7lq.XltI6LBhQlvslexzO5mRhynBgAInmkUcke1HdwvDTj0=")
#kc = LINE("ExkMVgvk1guSqmn6YB53.HH1fMZrnqDtM7+JKe+908W.OplI8bNnby0392bLhZTA7lXUF6dXdTp2Eo4UKGwrT9Y=")
#ke = LINE("ExukYnwcQxnfSeAvt6r7.LTK01ct2vjuMd/l3DlSnfW.bp+GoKRLrtNp50dzwM76N6aYI9k/CDHsQtczjmkysLU=")
print ("""
TEAM DELETE
╔══╗
║█████╗ 
...
admin=['u5a91f31a0882cae3e309576cc4bf1e5a']
...
Notify = "u0ca31a773d068463a3959eb50d00f662"
line.sendMessage(Notify,"เข้าสู่ระบบสำเร็จแล้ว")
#==============================================================================#
def RhyN_(to, mid):
    try:
...
#==============================================================================#
                    ...
                    line.sendContact(receiver, "ukdjsjxo7282827',")       
...
#==============================================================================#               
                    ...
                    line.sendMentionFooter(to, str(settings["me"]), sender, "https://line.me/ti/p/~botline2034", "http://dl.profile.line-cdn.net/"+line.getContact(sender).pictureStatus, line.getContact(sender).displayName);line.sendMessage(to, line.getContact(sender).displayName, contentMetadata = {'previewUrl': 'http://dl.profile.line-cdn.net/'+line.getContact(sender).pictureStatus, 'i-installUrl': 'https://line.me/ti/p/~botline2034', 'type': 'mt', 'subText': str(settings["icon"]), 'a-installUrl': 'https://line.me/ti/p/~botline2034', 'a-installUrl': ' https://line.me/ti/p/~botline2034', 'a-packageName': 'com.spotify.music', 'countryCode': 'ID', 'a-linkUri': 'https://line.me/ti/p/~botline2034', 'i-linkUri': 'https://line.me/ti/p/~botline2034', 'id': 'mt000000000a6b79f9', 'text': 'ᎠᎬᏞᎬᎢ', 'linkUri': 'https://line.me/ti/p/~botline2034'}, contentType=19)
                    line.sendImageWithURL(to, "https://www.img.live/image/DLtSu")
                elif text.lower() == 'คิก':
                   ...
                   elapsed_time = time.time() - start
                   line.sendMessage(to, "[ %s ความเร็วบอท ] [ " % (elapsed_time) + str(int(round((time.time() - start) * 500)))+" ปิง ]")
                ...
                elif text.lower() == 'ลบเพื่อน':
                    if line != None:
                        me = line.getContact(to)
                        line.deleteContact(to)
                        line.sendMessage(to, "ลบออกจากการเป็นเพื่อนแล้ว!")
                elif text.lower() == 'เพิ่มเพื่อน':
                    if line != None:
                        me = line.getContact(to)
                        line.findAndAddContactsByMid(to)
                        line.sendMessage(to, "เพิ่มเพื่อนแล้ว!")                            
...
#==============================================================================#
                    ...
                    for x in range(jml):
                        if 'MENTION' in msg.contentMetadata.keys()!= None:
                            ...
                            for receiver in lists:
                                ...
                                pass
                elif msg.text.lower().startswith("รันแชท "):
                    sep = text.split(" ")
                    text = text.replace(sep[0] + " ","")
                    cond = text.split(" ")
                    jml = int(cond[0])
                ...
                elif msg.text.lower() == 'เช็คห้ามพิม':
                    tst = "คำห้ามพิม:\n"
                    if len(wbanlist) > 0:
                    ...
                    settings["taggift"] = True
                    line.sendMessage(msg.to,"เปิดระบบส่งของขวัญไห้คนแทค(○ﾟεﾟ○)")
                
                elif msg.text in ["ปิดแทค3","Tag3 off"]:
                    settings["taggift"] = False
                    line.sendMessage(msg.to,"ปิดระบบส่งของขวัญไห้คนแทค(○ﾟεﾟ○)")
                    
                elif msg.text in ["เปิดแทค4","Tag4 on"]:
                    settings["delayMention"] = True
                    line.sendMessage(msg.to,"เปิดระบบแทคกลับคนแทค(○ﾟεﾟ○)")                
                elif msg.text in ["ปิดแทค4","Tag4 off"]:
                    settings["delayMention"] = False
                    line.sendMessage(msg.to,"ปิดระบบแทคกลับคนแทค(ˉ(∞)ˉ)")
                elif msg.text in ["เปิดแทค5","Tag5 on"]:
                    settings["fuck"] = True
                    line.sendMessage(msg.to,"เปิดระบบส่งคทไวรัส(○ﾟεﾟ○)")                
                elif msg.text in ["ปิดแทค5","Tag5 off"]:
                    settings["fuck"] = False
                    line.sendMessage(msg.to,"ปิดระบบส่งคทไวรัส(ˉ(∞)ˉ)")
                    ...
                    if settings["taggift"] == True:
                        ...
                        for mention in mentionees:
                            if lineMID in mention["M"]:
                                ...
                                break
                if 'MENTION' in msg.contentMetadata.keys()!= None:
                    if settings["fuck"] == True:
                        names = re.findall(r'@(\w+)', text)
                        mention = ast.literal_eval(msg.contentMetadata['MENTION'])
                        mentionees = mention['MENTIONEES']
                        for mention in mentionees:
                            if lineMID in mention["M"]:
                                line.sendContact(to, "ukdjsjxo7282827',")
                                break
                         ...
                         if settings['detectMention'] == True:                    
                             ...
                             for mention in mentionees:
                                   if mention['M'] in lineMID:
                                          ...
                                          sendMention(to, sender, "「ตอบแทคอัตโนมัติ」\n", "\n" + str(settings["Respontag"]))
                                          msgSticker = settings["messageSticker"]["listSticker"]["คนแทค"]
                                          if msgSticker != None:
                                              sid = msgSticker["STKID"]
                                              spkg = msgSticker["STKPKGID"]
